...
import { updateCategory, bulkImportCategoryProducts } from "@/actions/categories";
...
export default function EditCategoryForm({ category, allProducts }: { category: Category; allProducts: any[] }) {
  ...
  const [selected, setSelected] = useState<Product[]>(
    category.products.map((cp) => ({
      ...
      nameEnglish: cp.product.nameEnglish,
      unit: (cp.unit || cp.product.unit) as Unit,
      price: cp.price || cp.product.price,
      proof: cp.proof || cp.product.proof || "",
    }))
  );
  ...
  const filtered = allProducts.filter(
    (p) => !selected.find((s) => s.productId === p.id) &&
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Edit Category & Products</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 space-y-6">
        ...
        <div className="pt-4 border-t border-gray-50">
          <label className="block font-semibold text-gray-700 mb-2 hindi-text">आणखी उत्पादने जोडा</label>
